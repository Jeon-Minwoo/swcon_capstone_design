...
def processing_whitebg(img_name):
    ...
    if roi_circle[2] != -1:
        roi_rect = roi_rect.astype(np.int32)
...
def processing_checkpat(img_name):
    img_rgb = cv.imread(img_name)
    img_gray = cv.cvtColor(img_rgb, cv.COLOR_BGR2GRAY)
...
def all_images_run():
    image_list = [img for img in os.listdir('./eval') if '.jpeg' or '.png' in img]
    ...
    for i in image_list:
        ...
        if i[8:10] == '01':
            ...
            result1=processing_whitebg(path)
            ...
            with open(f'./solution_info/{i[:7]}-{i[14:16]}.yml', 'a') as outfile:
                yaml.dump(solution_info, outfile, default_flow_style=False)
            
        elif i[8:10] == '02':
            print("processing checkptn --", i)
            result2=processing_checkpat(path)
            supp_ratio = result2
            solution_info = dict(
                Name = i[:7],
                aug_idx = i[14:16],
            ...
            #print(solution_info)
            with open(f'./solution_info/{i[:7]}-{i[14:16]}.yml', 'w') as outfile:
                yaml.dump(solution_info, outfile, default_flow_style=False)

def yaml2csv():
    info_file_name = os.listdir('./solution_info')
    info_file_name = sorted(info_file_name)
    print(info_file_name)
    cnt = 0
    for f in info_file_name:
        with open(f'./solution_info/{f}') as yml_info:
            info = yaml.safe_load(yml_info)
            info_row = [info['Name'], info['aug_idx'], info['check_detection']['supp_ratio'], info['Grid_homogeneity']['mmg'], info['Grid_homogeneity']['msg'],
                        info['Grid_homogeneity']['smg'], info['Grid_homogeneity']['ssg'], info['Profile']['area'], info['Profile']['curve_c'], 
                        info['Profile']['gradient'], info['Segmentation']['particle_sum'], info['target']]
        if cnt == 0:
            with open("./solution_data.csv", 'w') as data:
                wr = csv.writer(data, lineterminator='\n')
                wr.writerow(['Name', 'aug_idx','supp_ratio', 'mmg', 'msg', 'smg', 'ssg', 'area','curve_c', 'gradient', 'particle_sum', 'target'])
                wr.writerow(info_row)
        else:
            with open("./solution_data.csv", 'a') as data:
                wr = csv.writer(data, lineterminator='\n')
                wr.writerow(info_row)
        cnt +=1  
