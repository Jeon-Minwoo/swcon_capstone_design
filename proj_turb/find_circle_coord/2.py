...
def make_dot_img(resol_x: int, resol_y: int, x_dot, y_dot):
    ...
    #꼭지점 원 생성
    # r = 30
    # cv2.circle(dot_img, (r,r), r, (255,0,0), -1) #left top blue circle
    # cv2.circle(dot_img, (r,resol_y-r), r, (0,255,0), -1)
    # cv2.circle(dot_img, (resol_x-r,r), r, (0,255,0), -1)
    # cv2.circle(dot_img, (resol_x-r,resol_y-r), r, (0,255,0), -1)
...
def find_corner(img):  #input은 휴대폰으로 촬영한 사진
    ...
    _, imthres = cv2.threshold(img_gray, 100, 255, cv2.THRESH_BINARY)
    imthres = cv2.GaussianBlur(imthres,(3,3), 0)
    #imthres = cv2.bilateralFilter(imthres,9,75,75)
    imthres = cv2.edgePreservingFilter(imthres, flags=1, sigma_s=45, sigma_r=0.2)
    
    #edge detection
    img_edged = cv2.Canny(imthres, 50, 200)
    ...
    cv2.drawContours(img_, [approx], -1, (0,255,0), 4)
    cv2.imshow('con', img_)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
if __name__ == "__main__":
    ...
    labtop_dot = cv2.imread('./img/labtop_dot5.jpg')
